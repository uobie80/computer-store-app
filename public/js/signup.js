const signupFormHandler = async (event) => {
    ...
    var membership = document.querySelector('#gridCheck').checked ? "premium" : "free";
      ...
      console.log(JSON.stringify({username, password, email, firstname, lastname, phone, address, address2, state, city, zip, country, membership}));

    if (firstname && lastname && username && email && password) {
      const response = await fetch('/users', { 
        method: 'POST', 
        body: JSON.stringify({username, password, email, firstname, lastname, phone, address, address2, state, city, zip, country, membership}),
